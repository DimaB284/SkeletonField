        ...
        if (agent.player != null && agent.firePoint != null && agent.bulletPrefab != null && agent.IsTargetInFOV(agent.player))
        {
            ...
            if (!inBurst)
            {
                if (Time.time >= nextBurstTime)
                {
                    inBurst = true;
                    burstCount = 0;
                    burstSize = Random.Range(3, 6); // випадкова довжина серії
                    shootCooldown = Random.Range(0.09f, 0.15f); // випадковий інтервал між пострілами
                    burstCooldown = Random.Range(0.8f, 1.3f); // випадкова пауза між серіями
                }
                else
                {
